    ...
    def walk(node, depth, result=""):
        for item in node:
            ...
            if item['action'] == 'deleted':
                result +=  f"{indent}- {item['name']}: {str(to_str(item['value'], new_depth))}"
            elif item['action'] == 'added':
                result += f"{indent}+ {item['name']}: {str(to_str(item['value'], new_depth))}"
            elif item['action'] == 'nested':
                result += f"{indent}  {item['name']}: {walk(item['children'], new_depth, result="")}"
            elif item['action'] == 'modified':
                result += f"{indent}- {item['name']}: {str(to_str(item['old_value'], new_depth))}\n"
                result += f"{indent}+ {item['name']}: {str(to_str(item['new_value'], new_depth))}"
